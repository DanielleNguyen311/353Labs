...
def pivot_months_pandas(data):
    ...
    #add a new column named 'month' to the 'data' data frame.
    #apply 'date_to_month' function to each value in 'date' column
    #and add the result to the 'month' column
    data['month'] = data['date'].apply(date_to_month)
    #Method 2 to add a new month column and apply date_to_month to each row : Using data.index.map(), it assumes the index of DataFrame is a Datetimeindex
    #and map the function to each row
    # data['month'] = data.index.map(date_to_month)
    
    
    #group the original data by name and month, 
    #aggregate the precipitation column, set the precipitation to sum to calculate the sum of precipitation values for each group
    #reset index reset the grouped index of the groued DataFrame into regular integer indexing
    monthly = data.groupby(['name','month']).aggregate({'precipitation':'sum'}).reset_index()
    
    #double check this: use panda pivot method to create name for row, month for columns
    monthly = monthly.pivot(index='name',columns='month',values='precipitation')
    
    #aggregate the precipitation column, set the precipitation to count to calculate the count of precipitation observations for each group
    counts = data.groupby(['name','month']).aggregate({'precipitation':'count'}).reset_index()
    #double check this: use panda pivot method to create name for row, month for columns
    monthly = monthly.pivot(index='name',columns='month',values='precipitation')
...
def main():
    ...
    totalsPanda, countsPanda = pivot_months_loops(data)

    totals.to_csv('totals.csv')
    counts.to_csv('counts.csv')
    
    totalsPanda.to_csv('totalsPanda.csv')
    countsPanda.to_csv('countsPanda.csv')
